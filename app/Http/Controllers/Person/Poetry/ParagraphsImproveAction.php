...
class ParagraphsImproveAction
{
    public function __invoke(int $life_id, string $llm, ChapterToLlmRequest $request)
    {
        if (!$life = Life::whereId($life_id)->with(['person', 'poetry'])->first()) {
            return redirect(route('web.person.list'));
        }
        $poetry = $life->poetrySpecific(LL_ENG, $llm);
        ...
        $nextLlm = $request->from_llm . '_' . $response->first()->llm . '.improve';
